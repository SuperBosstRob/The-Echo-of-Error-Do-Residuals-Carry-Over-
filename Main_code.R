...
# 1. OBJECTIVE FUNCTION THAT RETURNS SUM OF SQUARED ERRORS (SSE)
...
# 5. FUNCTION TO FIT MODELS FOR A SINGLE PARTICIPANT (USING SSE)
  ...
  tryCatch({
    de_restricted <- DEoptim(
      ...
      control = DEoptim.control(
        itermax = 500,  # Increased iterations
        trace = FALSE,
        strategy = 2,
        NP = 200,       # Increased population
        parallelType = 0,
        reltol = 1e-8,
        steptol = 100
      )
