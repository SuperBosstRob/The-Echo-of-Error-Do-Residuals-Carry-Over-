...
#         function(x) x
#     ),
#     "quasi_hyperbolic" = list(
#         simulate_quasi_hyperbolic,
#         quasi_hyperbolic, 
#         cbind(
#             c(0, -5, 1e-5, 1e-5, 1e-5),
#             c(1, 5, 1 - 1e-5, 1 - 1e-5, 0.25)
#         ),
#         function(x) x
#     ),
#     "double_exponential" = list(
#         simulate_double_exponential,
#         double_exponential, 
#         cbind(
#             c(0, -5, 1e-5, 1e-5, 1e-5, 1e-5),
#             c(1, 5, 1 - 1e-5, 1 - 1e-5, 0.5, 0.25)
#         ),
#         function(x) x
#     ),
#     "armax_restricted" = list(
#         \(parameters, X) simulate_armax(parameters, X, restricted = TRUE),
#         \(parameters, y, X) armax(parameters, y, X, restricted = TRUE),
#         cbind(
#             c(0, -5, -(1 - 1e-5), 1e-5),
#             c(1, 5, 1 - 1e-5, 0.25)
#         ),
...
#     ),
#     "armax" = list(
#         \(parameters, X) simulate_armax(parameters, X, restricted = FALSE),
#         \(parameters, y, X) armax(parameters, y, X, restricted = FALSE), 
#         cbind(
#             c(0, -5, -(1 - 1e-5), -(1 - 1e-5), 1e-5),
#             c(1, 5, 1 - 1e-5, 1 - 1e-5, 0.25)
#         ),
#         function(x) x
#     ),
#     "varmax_exponential" = list(
#         \(parameters, X) simulate_varmax(parameters, X, restricted = TRUE),
#         \(parameters, y, X) exponential(parameters, y, X),
#         cbind(
#             c(0, -5, -(1 - 1e-5), 1e-5),
#             c(1, 5, 1 - 1e-5, 0.25)
#         ),
#         function(x) c((1 - x[3]) * x[1], x[2], x[3], x[4])
#     ),
#     "exponential_varmax" = list(
#         \(parameters, X) simulate_exponential(parameters, X),
#         \(parameters, y, X) varmax(parameters, y, X, restricted = TRUE),
#         cbind(
#             c(0, -5, -(1 - 1e-5), 1e-5),
#             c(1, 5, 1 - 1e-5, 0.25)
#         ),
#         function(x) c((1 - x[3])^(-1) * x[1], x[2], x[3], x[4])
#     )
# )
models <- list(
    "varmax" = list(
...
for(i in seq_along(models)) {
    ...
    cat(paste0("Results saved successfully to: ", output_file, "\n"))
    ...
    figure_file <- file.path("figures", paste0(names(models)[i], ".png"))
    cat(paste0("Saving figure to: ", figure_file, "\n"))
    ...
    cat(paste0("Figure saved successfully to: ", figure_file, "\n"))
}
